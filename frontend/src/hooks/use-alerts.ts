  ...
  useEffect(() => {
    ...
    if (savedAlerts) {
      try {
        ...
        const alertsWithDates = parsed.map((alert: unknown) => {
          const alertObj = alert as Record<string, unknown>;
          return {
            ...alertObj,
            createdAt: new Date(alertObj.createdAt as string),
            triggeredAt: alertObj.triggeredAt ? new Date(alertObj.triggeredAt as string) : undefined,
          };
        });
