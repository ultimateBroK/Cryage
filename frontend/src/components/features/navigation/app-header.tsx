...
export const AppHeader: React.FC<HeaderSectionProps> = ({ 
  ...
  return (
    <header className="flex h-14 sm:h-16 shrink-0 items-center gap-2 sm:gap-3 bg-background/40 backdrop-blur-xl border-b border-white/20 pl-2 sm:pl-4 pr-0 sticky top-0 z-20 shadow-lg safe-area-inset-top">
      {/* Left side - Sidebar Trigger */}
      <div className="flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 flex-shrink-0 touch-target">
      ...
      <div className="flex-1 flex justify-center items-center px-2 sm:px-4 min-w-0">
        <div className="flex items-center bg-background/60 backdrop-blur-xl border border-white/25 rounded-xl p-1 sm:p-1.5 gap-1 sm:gap-2 shadow-lg">
          <button
            ...
            className={`flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-sm sm:text-base font-medium transition-all duration-300 ease-out transform-gpu touch-target relative ${
              isMobile ? 'w-[70px]' : deviceType === 'tablet' ? 'w-[100px]' : 'w-[120px]'
            } ${
              activeTab === "chat"
          ...
          >
            ...
            {unreadMessageCount > 0 && (
              <span className={`absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 bg-destructive text-destructive-foreground text-xs rounded-full flex items-center justify-center animate-pulse font-bold ${
                isMobile ? 'w-4 h-4 text-[9px]' : 'w-5 h-5 sm:w-6 sm:h-6 text-[10px] sm:text-xs'
              }`}>
          ...
          >
            ...
            {systemNotificationCount > 0 && (
              <span className={`absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 bg-secondary text-secondary-foreground text-xs rounded-full flex items-center justify-center animate-pulse font-bold ${
                isMobile ? 'w-4 h-4 text-[9px]' : 'w-5 h-5 sm:w-6 sm:h-6 text-[10px] sm:text-xs'
              }`}>
      ...
      {/* Right side - Settings */}
      <div className="flex items-center justify-center h-8 sm:h-9 flex-shrink-0 pr-2 sm:pr-4">
