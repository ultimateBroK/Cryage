...
const DashboardSidebar: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 space-y-2.5 p-2 sm:p-3 will-change-transform overflow-y-auto">
      ...
      >
        <Card className="relative overflow-hidden bg-gradient-to-br from-blue-50/80 to-cyan-50/80 dark:from-slate-900/60 dark:to-blue-900/30 border-blue-200/60 dark:border-blue-800/40 shadow-md hover:shadow-lg transition-all duration-300 group">
          ...
          <CardHeader className="pb-2 relative">
            <CardTitle className="text-xs sm:text-sm flex items-center justify-start gap-1.5 text-blue-700 dark:text-blue-300 font-medium">
          ...
          <CardContent className="space-y-2.5 relative">
            <div className="grid grid-cols-2 gap-2">
              <div className="text-center p-2 rounded-lg bg-white/60 dark:bg-slate-800/30 border border-blue-100/50 dark:border-blue-800/20">
            ...
            <div className="flex justify-between items-center p-2 rounded-lg bg-white/60 dark:bg-slate-800/30 border border-blue-100/50 dark:border-blue-800/20">
              <span className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-400 truncate">BTC Dominance</span>
              <span className="text-xs sm:text-sm font-bold text-blue-800 dark:text-blue-300">{marketStats.btcDominance}</span>
          </div>
            
            {/* Fear & Greed Index with visual indicator */}
            <div className="p-2.5 rounded-lg bg-white/60 dark:bg-slate-800/30 border border-blue-100/50 dark:border-blue-800/20 space-y-2">
          <div className="flex justify-between items-center">
                <span className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-400">Fear & Greed</span>
                <span className="text-[10px] sm:text-xs font-medium text-slate-500 dark:text-slate-400">{getFearGreedLabel(marketStats.fearGreedIndex)}</span>
          </div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs sm:text-sm font-bold text-slate-700 dark:text-gray-300 min-w-[20px] text-center">{marketStats.fearGreedIndex}</span>
              </div>
            </div>
          </CardContent>
         </Card>
        ...
        <Card className={`relative overflow-hidden ${showMovers 
          ...
          <CardHeader className="pb-2 relative">
            <div className="flex items-center justify-between gap-1">
              <CardTitle className={`text-xs sm:text-sm flex items-center gap-1.5 ${showMovers 
              ...
              <Button
                ...
                className={`h-6 px-2 text-[10px] sm:text-xs font-medium transition-all duration-200 flex-shrink-0 ${showMovers 
          ...
          <CardContent className="space-y-2 relative">
            {currentData.map((mover, index) => (
              <MotionDiv
                ...
                className={`flex justify-between items-center p-2 rounded-lg transition-all duration-200 cursor-pointer group/item border ${showMovers 
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-xs sm:text-sm font-bold text-slate-800 dark:text-gray-200 min-w-[32px] truncate">{mover.symbol}</span>
                  {mover.isPositive ? (
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] sm:text-xs text-slate-600 dark:text-slate-400 min-w-[45px] text-right truncate">{mover.price}</span>
                  <span className={`text-[10px] sm:text-xs font-semibold px-2 py-0.5 rounded-full min-w-[45px] text-center ${
                    mover.isPositive 
      ...
      >
          <Card className="relative overflow-hidden bg-gradient-to-br from-orange-50/80 to-red-50/80 dark:from-slate-900/60 dark:to-orange-900/30 border-orange-200/60 dark:border-orange-800/40 shadow-md hover:shadow-lg transition-all duration-300 group">
            <div className="absolute inset-0 bg-gradient-to-br from-orange-500/3 to-red-500/3 dark:from-orange-400/3 dark:to-red-400/3" />
            <CardHeader className="pb-2 relative">
              <CardTitle className="text-xs sm:text-sm flex items-center justify-start gap-1.5 text-orange-800 dark:text-orange-300 font-medium">
                ...
                <Badge variant="destructive" className="text-[10px] sm:text-xs px-1.5 py-0.5 bg-orange-600 text-white dark:bg-orange-600 border border-orange-200/60 dark:border-orange-800/30 flex-shrink-0">
            ...
            <CardContent className="space-y-2 relative">
              <MotionDiv
                ...
                className="flex items-center gap-2 p-2 rounded-lg bg-white/60 dark:bg-slate-800/30 hover:bg-orange-50/80 dark:hover:bg-orange-900/10 transition-all duration-200 cursor-pointer border border-orange-100/50 dark:border-orange-800/20"
              >
                <Activity className="w-3 h-3 sm:w-4 sm:h-4 text-green-600 dark:text-green-400 animate-pulse flex-shrink-0" />
                <span className="text-[10px] sm:text-xs text-orange-800 dark:text-orange-300 font-medium truncate">BTC above $45,000</span>
              </MotionDiv>
              <MotionDiv
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              >
                ...
                <span className="text-[10px] sm:text-xs text-orange-800 dark:text-orange-300 font-medium truncate">ETH RSI oversold</span>
              </MotionDiv>
              <MotionDiv
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              >
                ...
                <span className="text-[10px] sm:text-xs text-orange-800 dark:text-orange-300 font-medium truncate">SOL volume spike</span>
              </MotionDiv>
         </CardContent>
       </Card>
      </MotionDiv>
