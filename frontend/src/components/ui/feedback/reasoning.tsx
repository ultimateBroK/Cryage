...
import StarBorder from "@/components/ui/layout/star-border";
import {
...
import { preventScrollGently } from "@/utils/gentle-scroll-prevention";
...
export const ReasoningTrigger: FC<PropsWithChildren<{ className?: string }>> = ({ className, children }) => {
  ...
  const { setReasoningAnimating, restoreChatScrollAfterReasoning } = useScrollIsolation();
  ...
  const handleToggle = (event: React.MouseEvent) => {
    ...
    // Signal that reasoning animation is starting
    setReasoningAnimating(true);
    
    // Use gentle scroll prevention to avoid shaking
    const cleanup = preventScrollGently(450);
    
    setOpen(newOpenState);
    
    // Restore chat scroll system after animation completes
    setTimeout(() => {
...
export const ReasoningContent: FC<PropsWithChildren<{ markdown?: boolean; className?: string }>> = memo(({
  ...
  const { canReasoningScroll, setReasoningAnimating } = useScrollIsolation();
  ...
  useEffect(() => {
    if (open) {
      ...
      const timer = setTimeout(() => {
        setReasoningAnimating(false);
      }, 500);
      return () => clearTimeout(timer);
    } else {
      setReasoningAnimating(false);
    }
  }, [open, setReasoningAnimating]);
    ...
    return open ? (
      ...
      >
        <div
          ref={scrollRef}
          className={cn(
            "reasoning-content inline-block rounded-xl bg-background/40 backdrop-blur-md border border-white/15 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.25)] p-3 text-left mx-auto transition-all duration-400",
      ...
      {open && (
        ...
        >
          <MotionDiv
            ref={scrollRef}
            className={cn(
              "reasoning-content inline-block rounded-xl bg-background/40 backdrop-blur-md border border-white/15 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.25)] p-3 text-left mx-auto transition-all duration-200",
